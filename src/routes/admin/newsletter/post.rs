...
pub async fn publish_newsletter(
    ...
    let issued_id = insert_newsletter_issue(
        &mut transaction,
        &title,
        &text_content,
        &html_content,
    ...
    .await
    .context("Failed to store newsletter issue in the details")
    .map_err(e500)?;
    enqueue_delivery_tasks(&mut transaction, issued_id)
        .await
        .context("Failed to enqueue delivery tasks")
        .map_err(e500)?;
    
    let response = see_other("/admin/newsletters");
    let response = save_response(transaction, &pool, &idempotency_key, *user_id, response)
        .await
        .map_err(e500)?;
    success_message().send();
    Ok(response)
